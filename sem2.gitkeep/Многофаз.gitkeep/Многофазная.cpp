...
void multi(const string& inputFile, int k = 3)
{
    ...
    for (int i = 0; i < k; ++i)
        tapes[i] = "D:\\2 сем\\Многофазная\\int" + to_string(i) + ".txt";

    distrub(inputFile, tapes, k);
    string tempFile = "D:\\2 сем\\Многофазная\\inp.txt";
    int activTape = k;
    ...
    for (int i = 0; i < k; ++i) {
        ...
        if (f.peek() != ifstream::traits_type::eof()) {
            f.close();
            remove("D:\\2 сем\\Многофазная\\in_sorted.txt");
            if (rename(tapes[i].c_str(), "D:\\2 сем\\Многофазная\\in_sorted.txt") != 0)
...
int main()
{
    ...
    const int n = 20;
    int mas[n];
    ofstream file("D:\\2 сем\\Многофазная\\in.txt");
    for (int i = 0; i < n; i++)
    {
        mas[i] = rand() % 100 - 50;
        file << mas[i] << endl;
    ...
    cout << endl << endl;
    multi("D:\\2 сем\\Многофазная\\in.txt", 15);
    ifstream sort("D:\\2 сем\\Многофазная\\in_sorted.txt");
    cout << "Результат" << endl;
    int k;
    while (sort >> k)
    {
